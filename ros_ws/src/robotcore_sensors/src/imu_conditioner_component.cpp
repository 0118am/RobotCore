...
private:
  ...
  void load_persistent_calibration()
  {
    ...
    try {
      ...
      // Never apply coefficients from a file that failed identity, hash, or
      // numeric validation. Startup gyro calibration remains available, while
  ...
  void on_imu(const sensor_msgs::msg::Imu::SharedPtr message)
  {
    ...
    bool accel_enabled = false;
    {
      ...
      if (!ready_) {
        ...
        if (gyro_stationary && (!persistent_calibration_valid_ || accel_stationary)) {
    ...
    const Eigen::Vector3d base_zeroed_accel = rotate_imu_vector_to_base(zeroed_accel, base_from_imu_);
    const Eigen::Vector3d base_fusion_accel = rotate_imu_vector_to_base(fusion_accel, base_from_imu_);
    ...
    output.linear_acceleration.x = base_zeroed_accel.x();
    output.linear_acceleration.y = base_zeroed_accel.y();
    output.linear_acceleration.z = base_zeroed_accel.z();
    ...
    fusion_output.linear_acceleration.x = base_fusion_accel.x();
    fusion_output.linear_acceleration.y = base_fusion_accel.y();
    fusion_output.linear_acceleration.z = base_fusion_accel.z();
  ...
  void finish_calibration()
  {
    ...
    accel_enabled_ = acceleration_fusion_enabled(
      persistent_calibration_valid_, startup_accel_residual_.norm(), 1.5);
    ready_ = true;
    collecting_ = false;
    publish_ready();
  ...
  {
    std::lock_guard<std::mutex> lock(state_mutex_);
    const int level = ready_ ?
      (accel_enabled_ ? diagnostic_msgs::msg::DiagnosticStatus::OK :
      diagnostic_msgs::msg::DiagnosticStatus::WARN) :
      (timed_out_ ? diagnostic_msgs::msg::DiagnosticStatus::ERROR : diagnostic_msgs::msg::DiagnosticStatus::WARN);
    const std::string message = ready_ ?
      (accel_enabled_ ? "calibrated" : "gyro calibrated; acceleration fusion disabled") :
  ...
  std::string input_topic_, output_topic_, fusion_output_topic_, status_topic_, base_frame_id_;
