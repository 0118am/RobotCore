...
from robotcore_interfaces.msg import AprilTagPoseStatus
...
class AprilTagLocalizationNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("min_tag_camera_z_m", 0.10)
    ...
    def pnp_observation_is_credible(
        ...
        minimum_camera_z_m = max(
            0.0, float(self.runtime_parameters["min_tag_camera_z_m"])
        )
        tag_camera_points = camera_points.reshape(-1, 4, 3)
        invalid_camera_z_ids = [
            int(tag_id)
            for tag_id, points in zip(seen_ids, tag_camera_points)
            if float(np.min(points[:, 2])) <= minimum_camera_z_m
        ]
        if invalid_camera_z_ids:
            self.warn_throttled(
                "invalid-tag-camera-z",
                "AprilTag PnP placed mapped Tags "
                f"{invalid_camera_z_ids} behind or too close to the camera",
            )
            return False
