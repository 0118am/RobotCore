...
namespace
{
eup_sensors::FixedLagEskf stationary_filter()
{
  eup_sensors::FixedLagEskf filter;
  eup_sensors::EskfState state;
  state.covariance.setIdentity();
  eup_sensors::ImuSample first;
...
TEST(FixedLagEskf, JosephUpdateKeepsCovariancePositive)
{
  ...
  Eigen::SelfAdjointEigenSolver<eup_sensors::Matrix15d> solver(filter.state().covariance);
...
TEST(FixedLagEskf, So3ExponentialAndLogarithmRoundTrip)
{
  ...
  const auto recovered = eup_sensors::log_quaternion(
    eup_sensors::exp_quaternion(rotation_vector));
...
TEST(FixedLagEskf, DelayedUpdateReplayMatchesChronologicalProcessing)
{
  ...
  std::vector<eup_sensors::ImuSample> samples;
  samples.reserve(50);
  eup_sensors::EskfState rewind_state;
  eup_sensors::ImuSample rewind_imu;
