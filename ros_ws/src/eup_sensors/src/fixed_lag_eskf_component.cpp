...
#include <eup_interfaces/msg/april_tag_pose_status.hpp>
#include <eup_interfaces/msg/body_state.hpp>
#include <eup_interfaces/msg/localization_status.hpp>
...
public:
  ...
  : Node("fixed_lag_eskf", options), updater_(this)
  {
    ...
    tag_status_sub_ = create_subscription<eup_interfaces::msg::AprilTagPoseStatus>(
    ...
    body_pub_ = create_publisher<eup_interfaces::msg::BodyState>("/robot/body_state", rclcpp::QoS(10).reliable());
    status_pub_ = create_publisher<eup_interfaces::msg::LocalizationStatus>("/localization/status", rclcpp::QoS(10).reliable());
...
private:
  ...
  void on_vio(const nav_msgs::msg::Odometry::SharedPtr message)
  {
    ...
    last_vio_stamp_ns_ = measurement_stamp; last_vio_arrival_ns_ = now().nanoseconds();
    vio_transport_s_ = (last_vio_arrival_ns_ - measurement_stamp) * 1e-9;
    vio_arrivals_.push_back(last_vio_arrival_ns_); prune_arrivals(vio_arrivals_, last_vio_arrival_ns_);
    if (!filter_.initialized()) {
      ...
      history_high_water_ = std::max(history_high_water_, history_.size()); return;
    }
    ...
    if (imu_stale && measurement_stamp > filter_.state().stamp_ns) {
      EskfState bridge_state = filter_.state();
      ImuSample bridge_imu = filter_.last_imu();
    ...
    if (insertion != vio_measurements_.end() && insertion->stamp == event.stamp) {*insertion = event;}
    else {vio_measurements_.insert(insertion, event);}
    replay_from(*index);
    ++fixed_lag_replays_;
  }

  void apply_vio_measurement(const VioMeasurement & measurement)
  {
    const bool pose_ok = filter_.update_pose(
      measurement.position, measurement.orientation, measurement.pose_covariance);
    const bool velocity_ok = filter_.update_velocity(
      measurement.velocity, measurement.velocity_covariance);
    if (!pose_ok || !velocity_ok) {++vio_gate_rejections_;}
  }

  void replay_from(std::size_t first)
  {
    ...
    for (std::size_t i = first; i < history_.size(); ++i) {
      ...
      for (const auto & event : vio_measurements_) {
        ...
        if (event_index && *event_index == i) {apply_vio_measurement(event);}
      }
  ...
  void on_tag_status(const eup_interfaces::msg::AprilTagPoseStatus::SharedPtr message)
  {
  ...
  void publish()
  {
    ...
    const double vio_age = last_vio_stamp_ns_ > 0 ? (now_ns - last_vio_stamp_ns_) * 1e-9 : INFINITY;
    ...
    nav_msgs::msg::Odometry odom; odom.header.stamp = now_time; odom.header.frame_id = map_frame_; odom.child_frame_id = base_frame_;
    ...
    eup_interfaces::msg::BodyState body; body.header = odom.header; body.pose = odom.pose.pose; body.twist = odom.twist.twist;
    body.linear_velocity_valid = vio_age <= inertial_horizon_s_; body.depth_m = std::numeric_limits<float>::quiet_NaN();
    body.altitude_m = std::numeric_limits<float>::quiet_NaN(); body.state_valid = absolute_valid;
  ...
  {
    eup_interfaces::msg::LocalizationStatus status; status.header.stamp = stamp; status.header.frame_id = map_frame_;
    if (last_vio_stamp_ns_ > 0) {status.last_vio_stamp = rclcpp::Time(last_vio_stamp_ns_, RCL_ROS_TIME);}
  ...
  std::int64_t last_vio_stamp_ns_{}, last_vio_arrival_ns_{}, last_imu_arrival_ns_{}, last_tag_stamp_ns_{};
  ...
  eup_interfaces::msg::AprilTagPoseStatus last_tag_status_;
  ...
  rclcpp::Subscription<eup_interfaces::msg::AprilTagPoseStatus>::SharedPtr tag_status_sub_;
  rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr fused_pub_, aligned_pub_;
  rclcpp::Publisher<eup_interfaces::msg::BodyState>::SharedPtr body_pub_;
  rclcpp::Publisher<eup_interfaces::msg::LocalizationStatus>::SharedPtr status_pub_;
...
RCLCPP_COMPONENTS_REGISTER_NODE(eup_sensors::FixedLagEskfComponent)
