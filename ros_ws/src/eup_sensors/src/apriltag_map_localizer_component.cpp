...
namespace
{
struct TagDefinition {std::array<Eigen::Vector3d, 4> corners; double size{};};

Eigen::Matrix3d rpy_rotation(const Eigen::Vector3d & rpy)
{
...
public:
  ...
  : Node("apriltag_localization", options)
  {
    ...
    map_file_ = declare_parameter<std::string>("tag_map_file", "");
    map_frame_ = declare_parameter<std::string>("map_frame", "map");
    base_frame_ = declare_parameter<std::string>("base_frame", "base_link");
    family_ = declare_parameter<std::string>("tag_family", "tag36h11");
    default_size_ = declare_parameter<double>("tag_size_m", 0.13);
    ...
    base_from_camera_.linear() = rpy_rotation({rpy[0], rpy[1], rpy[2]});
    load_map();
    ...
    status_pub_ = create_publisher<eup_interfaces::msg::AprilTagPoseStatus>(
...
private:
  bool load_map()
  {
    try {
      ...
      std::map<int, TagDefinition> replacement;
      for (auto iterator = root.at("tags").begin(); iterator != root.at("tags").end(); ++iterator) {
        const int id = std::stoi(iterator.key());
        ...
        const double size = iterator.value().value("size_m", default_size_);
        if (position_values.size() != 3U || rpy_values.size() != 3U || size <= 0.0 || size > 2.0) {
          throw std::runtime_error("invalid tag definition " + iterator.key());
        }
        if (replacement.count(id) != 0U) {
        ...
        const Eigen::Vector3d rpy = Eigen::Vector3d(rpy_values[0], rpy_values[1], rpy_values[2]) * M_PI / 180.0;
        if (!center.allFinite() || !rpy.allFinite()) {throw std::runtime_error("non-finite tag definition");}
        const auto rotation = rpy_rotation(rpy); const double half = 0.5 * size;
        const std::array<Eigen::Vector3d, 4> local = {
          Eigen::Vector3d(-half, half, 0), Eigen::Vector3d(half, half, 0),
          Eigen::Vector3d(half, -half, 0), Eigen::Vector3d(-half, -half, 0)};
        TagDefinition definition; definition.size = size;
        for (std::size_t i = 0; i < 4U; ++i) {definition.corners[i] = rotation * local[i] + center;}
        replacement[id] = definition;
      }
      if (replacement.empty()) {throw std::runtime_error("tag map is empty");}
      tags_ = std::move(replacement); map_error_.clear(); return true;
    ...
    std::shared_ptr<std_srvs::srv::Trigger::Response> response)
  {
    ...
    response->success = true; response->message = "tag map reloaded; next quality-gated pose bypasses transition gate";
  }
  ...
  {
    eup_interfaces::msg::AprilTagPoseStatus status; status.header = message->header;
  ...
  int min_tags_{}, min_inlier_corners_{}; double default_size_{}, min_edge_px_{}, max_rms_px_{}, max_reprojection_px_{};
  ...
  rclcpp::Publisher<eup_interfaces::msg::AprilTagPoseStatus>::SharedPtr status_pub_;
...
RCLCPP_COMPONENTS_REGISTER_NODE(eup_sensors::AprilTagMapLocalizerComponent)
