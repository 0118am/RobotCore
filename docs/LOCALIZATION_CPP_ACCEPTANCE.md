...
| 项目 | 结果 | 证据/命令 |
|---|---|---|
| Aquaboard 固件 Release 编译 | PASS | `cmake --build build-release --parallel 4`；`-O3 + LTO`，ELF/HEX/BIN 生成，Flash 26,404 B，RAM 21,096 B，BIN SHA-256 `ed64f3ccfa0045d6888088e652df6e193bd4298284b4dd885212fa7863b7cddb` |
...
| C++ EKF 融合测试 | PASS | 60 Hz 发布、仅融合 ZED VIO 与 Tag 位姿/twist；外置 IMU 不进入 EKF |
| 数据链运行语言 | PASS（静态） | UART/IMU 直发、Tag 地图 PnP、VIO/Tag 融合、BodyState 均为 C++；launch Python 只做进程编排 |
| 旧 `robot_localization` | PASS（静态） | edge launch 不再启动或依赖该节点，旧 YAML 不参与安装后的运行图 |
| 隔离域合成融合频率 | PASS（本机动态） | VIO 30 Hz、Tag 锚点输入；6 s 内 BodyState 336 条、59.998 Hz、时间戳严格递增、速度 0.2500 m/s |
| 部署后实时数据链 | PASS（真机动态） | Domain 42/CycloneDDS 图契约通过；外部 IMU 100.00 Hz、ZED odom 30.00 Hz、CUDA detections/RobotCore Tag pose 29.85 Hz、BodyState 60.00 Hz；核心状态话题均为单发布者 |
...
| 验收项 | 判据 | 状态 |
|---|---|---|
| 固件与主机原子升级 | 主机只接受 33-byte frame 4/version 2/CRC 正确数据；推进器物理断开或中位锁定刷写 | PENDING |
...
3. 检查 frame 4 计数器、MCU tick、CRC、attitude-valid、RPY 符号和到达间隔，再启动 edge launch。
